...
function togglePasswordVisibility() { 
    ...
    pass.type = confirm.type = isHidden ? "text" : "password"; toggle.innerHTML = isHidden ? "<strike>&#128065;</strike>" : "&#128065;"; 
} 
    function trigger2FA() { 
    ...
    if (!username) errors.push("Username cannot be empty."); 
        if (password.toLowerCase().includes(username.toLowerCase())) 
            errors.push("Don't use your username in password."); 
        ...
        const score = [ password.length >= 8 && password.length <= 32, /[a-z]/.test(password), /[A-Z]/.test(password), /[0-9]/.test(password), specialCount >= 2, !/\s/.test(password), !hasRepeatedChars(password), !/(abc|bcd|cde|123|234|345|456|567|678|789)/i.test(password), !password.toLowerCase().includes(username.toLowerCase()), password === confirm ].filter(Boolean).length; 
        ...
        if (errors.length) { result.style.color = "#AA0000"; result.textContent = errors.join("\n"); } 
        else { result.style.color = "#008800"; result.textContent = "Your Password is strong."; } 
    }
